...
def run(command):
  ...
  log.debug(f"{BLUE}Executing '%s'" % " ".join(command))
...
class Adapter:
  def __init__(self, iface):
    ...
    try:
      ...
      log.error(f"{RED}Unable to find adapter '%s', aborting." % iface)
      sys.exit(1)
    self.reset()
  ...
  def set_name(self, name):
    if self.adapter.Name != name:
      ...
      if name not in run(["hciconfig", self.iface, "name"]).decode():
        log.error(f"{RED}Unable to set adapter name, aborting.")
        sys.exit(1)
  ...
  def set_class(self, adapter_class):
    ...
    if self.adapter.Class != class_hex:
      ...
      if class_hex not in run(["hciconfig", self.iface, "class"]).decode():
        log.error(f"{RED}Unable to set adapter class, aborting.")
        sys.exit(1)
  ...
  def set_address(self, address):
    ...
    if address.upper() not in run(["hciconfig", self.iface]).decode():
      log.error(f"{RED}Unable to set adapter address, aborting.")
      sys.exit(1)
